...
import { CandlestickChart2 } from './CandlestickChart2';
import { useQuery } from 'react-query';
import { fetchPriceData, fetchVWPDataFreq, fetchFxMatikBoxLine, fetchGannData, fetchKahinAll, fetchSymbolData2 } from '../api/api';
import usePersistedState from '../hooks/usePersistedState';
import { KahinAll, SymbolData } from '../shared';
...
const GridKahinWeb = () => {
  ...
  const [selectedBoxes, setSelectedBoxes] = useState<{ [key: string]: boolean }>({
    Gann: true, Kahin: false, Fib: true
  });
  ...
  }, [inputSymbol, inputInterval, inputDayback]);
  ...
  useEffect(() => {
    const fetchSymbolData = async () => {
      try {
        const data = await fetchSymbolData2();
        setSymbolData(data);
      } catch (err) {
  ...
  const { data: vwpDataFreq, isLoading: vwpLoading, error: vwpError, refetch: refetchVWPDataFreq } = useQuery(
    ['vwp', queryParams.symbol, queryParams.interval, queryParams.dayback],
  ...
  }, [shouldFetch, refetchPriceData, refetchVWPDataFreq, refetchFxMatikBoxLine, refetchGannData]);
  ...
  useEffect(() => {
    ...
    const precision = symbolData.find(s => s.symbol_text === inputSymbol)?.price_precision;
  ...
  const columnDefs: ColDef[] = useMemo(() => [
    ...
    { field: 'datetime', headerName: 'Date', sortable: true, sort: "desc", filter: 'agDateColumnFilter', maxWidth: 100, valueFormatter: params => formatDateTime(params.value) },
    { field: 'market', headerName: 'M', sortable: true, filter: 'agTextColumnFilter', maxWidth: 60 },
    { field: 'is_viop', headerName: 'V', sortable: true, filter: 'agTextColumnFilter', maxWidth: 60 },
  ], []);
  ...
  const [symbolChangeInProgress, setSymbolChangeInProgress] = useState(false);
  // Handle row selection in AG-Grid
  const onRowSelected = (event: RowSelectedEvent) => {
    if (event.node.isSelected() && !symbolChangeInProgress) {
      const selectedSymbol = event.data.symbol;
      setSymbolChangeInProgress(true);
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          setSymbolChangeInProgress(false);
        }, 10);
      }, 10);
    }
  };
  ...
  useEffect(() => {
    const fetchKahinAllData = async () => {
      try {
        const data = await fetchKahinAll();
        setRowData(data.data);
        setLastUpdate(`K: ${formatTimestamp(data.last_update_kahin)} | G: ${formatTimestamp(data.last_update_gann)}`);
      } catch (err) {
        setError(err instanceof Error ? err.message : 'An unknown error occurred');
      } finally {
        setLoading(false);
  ...
  return (
    <div className="app-container">
      ...
      <div style={{ display: 'flex', width: '100%', height: '100%', gap: '2%' }}>
        <div className="ag-theme-alpine" style={{ height: dimensions.height * 0.8, width: '50%' }}>
          <AgGridReact
        ...
        <div id="kahinCandlediv" style={{ width: '50%', height: dimensions.height * 0.95 }}>
          <CandlestickChart2
            priceData={priceData || []}
            ...
            gannData={gannData || []}
            onCheckboxChange={handleCheckboxChangeChart}
